        ...
        async ({ comment_id }) => {
            try {
                ...
                if (comment.status !== 'sent') {
                    return {
                        content: [
                            {
                                type: 'text' as const,
                                text: `Error: Comment "${comment_id}" must be in 'sent' status to resolve. Current status: ${comment.status}.`,
                            },
                        ],
                    };
                }
                ...
                if (comment.resolved_at != null) {
                    return {
                        content: [
                            { type: 'text' as const, text: `Error: Comment "${comment_id}" is already resolved.` },
                        ],
                    };
                }

                db.run("UPDATE comments SET status = 'resolved', resolved_at = datetime('now') WHERE id = $id", {
                    $id: comment_id,
                });
