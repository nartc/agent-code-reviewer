...
export function registerCheckComments(server: McpServer, db: Database): void {
    server.registerTool('check_comments', {
        ...
        try {
            ...
            if (!repo && repo_name) {
                const repos = queryRows<RepoRow>(
                    db,
                    'SELECT id, name FROM repos WHERE name = $name',
                    { $name: repo_name },
                );
                repo = repos[0];
            }
            ...
            if (sessions.length === 0) {
                return {
                    content: [{ type: 'text' as const, text: `No unresolved comments found for repo "${repo.name}".` }],
                };
            }
            ...
            const lines: string[] = [`Found ${comments.length} unresolved comment${comments.length !== 1 ? 's' : ''} for repo "${repo.name}":\n`];
