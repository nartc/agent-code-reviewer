...
export class DbService {
  ...
  queryOne<T>(
    ...
    try {
      ...
      return err(
        databaseError(
          `QueryOne failed: ${e instanceof Error ? e.message : String(e)}`,
          e
        )
      );
    }
  ...
  execute(
    sql: string,
    params?: BindParams
  ): Result<{ changes: number }, AppError> {
    try {
  ...
  transaction<T>(fn: () => Result<T, AppError>): Result<T, AppError> {
    try {
      ...
      return err(
        databaseError(
          `Transaction BEGIN failed: ${e instanceof Error ? e.message : String(e)}`,
          e
        )
      );
    }
    ...
    if (result.isOk()) {
      try {
        ...
        return err(
          databaseError(
            `Transaction COMMIT failed: ${e instanceof Error ? e.message : String(e)}`,
            e
          )
        );
      }
